...
class TestCarRentalRules(unittest.TestCase):
    ...
    def test_return_rules(self):
        ...
        self.assertEqual(self._process(rules), dedent('''\
                                        ...
                                        :- in_country_rental(INCOUREN), actual_return_date_time(ACTRETDATTIM), has(in_country_rental(INCOUREN),actual_return_date_time(ACTRETDATTIM)), local_area(LOCARE), rented_car(RENCAR), not owns(local_area(LOCARE),rented_car(RENCAR)), in_country_rental(INCOUREN), return_branch(RETBRA), has(in_country_rental(INCOUREN),return_branch(RETBRA)), is_included_in(return_branch(RETBRA),local_area(LOCARE)), in_country_rental(INCOUREN), has(in_country_rental(INCOUREN),rented_car(RENCAR)).
                                        :- country(COU), country_of_registration(COUOFREG), COU != COUOFREG, return_branch(RETBRA), has(return_branch(RETBRA),country(COU)), international_inward_rental(INTINWREN), has(international_inward_rental(INTINWREN),return_branch(RETBRA)), international_inward_rental(INTINWREN), rented_car(RENCAR), has(international_inward_rental(INTINWREN),rented_car(RENCAR)), has(rented_car(RENCAR),country_of_registration(COUOFREG)).
                                        ...
                                        :- actual_return_date_time(ACTRETDATTIM), end_date_time(ENDDATTIM), ACTRETDATTIM > ENDDATTIM, rental(REN), has(rental(REN),actual_return_date_time(ACTRETDATTIM)), rental(REN), grace_period(GRAPER), has(rental(REN),grace_period(GRAPER)), has(grace_period(GRAPER),end_date_time(ENDDATTIM)), rental(REN), late_return_charge(LATRETCHA), not incurs(rental(REN),late_return_charge(LATRETCHA)).
                                        :- rental(REN), assigned(ASS), REN = ASS, rented_car(RENCAR), pick_up_branch(PICUPBRA), not is_stored_at(rented_car(RENCAR),pick_up_branch(PICUPBRA)), rental(REN), has(rental(REN),rented_car(RENCAR)), rental(REN), has(rental(REN),pick_up_branch(PICUPBRA)).
                                        :- drop_off_location(DROOFFLOC), eu_rent_site(EURENSIT), DROOFFLOC = EURENSIT, rental(REN), has(rental(REN),drop_off_location(DROOFFLOC)), rental(REN), return_branch(RETBRA), not has(rental(REN),return_branch(RETBRA)), is_base_for(eu_rent_site(EURENSIT),return_branch(RETBRA)), rental(REN), location_penalty_charge(LOCPENCHA), not incurs(rental(REN),location_penalty_charge(LOCPENCHA)).
                                        :- international_inward_rental(INTINWREN), actual_return_date_time(ACTRETDATTIM), has(international_inward_rental(INTINWREN),actual_return_date_time(ACTRETDATTIM)), local_area(LOCARE), rented_car(RENCAR), not owns(local_area(LOCARE),rented_car(RENCAR)), international_inward_rental(INTINWREN), return_branch(RETBRA), has(international_inward_rental(INTINWREN),return_branch(RETBRA)), is_included_in(return_branch(RETBRA),local_area(LOCARE)), international_inward_rental(INTINWREN), has(international_inward_rental(INTINWREN),rented_car(RENCAR)).
                                        ''').strip())
    ...
    def test_servicing_rules(self):
        rules = dedent('''\
                ...
                ''')
        self.assertEqual(self._process(rules), dedent('''\
                            :- rental_car(RENCAR), scheduled_service(SCHSER), not has(rental_car(RENCAR),scheduled_service(SCHSER)), in_need_of_service(INNEEOFSER), is(rental_car(RENCAR),in_need_of_service(INNEEOFSER)).
                            :- service_reading(SERREA), SERREA >= 5500, rental_car(RENCAR), has(rental_car(RENCAR),service_reading(SERREA)).
                            :- rented_car(RENCAR), in_need_of_service(INNEEOFSER), RENCAR = INNEEOFSER, rental(REN), has(rental(REN),rented_car(RENCAR)), open(OPE), is(rental(REN),open(OPE)), rental(REN), car_exchange_during_rental(CAREXCDURREN), not incurs(rental(REN),car_exchange_during_rental(CAREXCDURREN)).
                            :- rented_car(RENCAR), in_need_of_repair(INNEEOFREP), RENCAR = INNEEOFREP, rental(REN), has(rental(REN),rented_car(RENCAR)), open(OPE), is(rental(REN),open(OPE)), rental(REN), car_exchange_during_rental(CAREXCDURREN), not incurs(rental(REN),car_exchange_during_rental(CAREXCDURREN)).
                            ''').strip())
    ...
    def test_transfer_rules(self):
        ...
        self.assertEqual(self._process(rules), dedent('''\
                                    :- car_transfer(CARTRA), transfer_drop_off_date_time(TRADROOFFDATTIM), has(car_transfer(CARTRA),transfer_drop_off_date_time(TRADROOFFDATTIM)), transferred_car(TRACAR), local_area(LOCARE), not is_owned_by(transferred_car(TRACAR),local_area(LOCARE)), car_transfer(CARTRA), has(car_transfer(CARTRA),transferred_car(TRACAR)), car_transfer(CARTRA), transfer_drop_off_branch(TRADROOFFBRA), has(car_transfer(CARTRA),transfer_drop_off_branch(TRADROOFFBRA)), includes(local_area(LOCARE),transfer_drop_off_branch(TRADROOFFBRA)).
                                    ...
                                    :- international_return(INTRET), transfer_drop_off_date_time(TRADROOFFDATTIM), has(international_return(INTRET),transfer_drop_off_date_time(TRADROOFFDATTIM)), transferred_car(TRACAR), local_area(LOCARE), not is_owned_by(transferred_car(TRACAR),local_area(LOCARE)), international_return(INTRET), has(international_return(INTRET),transferred_car(TRACAR)), international_return(INTRET), transfer_drop_off_branch(TRADROOFFBRA), has(international_return(INTRET),transfer_drop_off_branch(TRADROOFFBRA)), includes(local_area(LOCARE),transfer_drop_off_branch(TRADROOFFBRA)).
                                    ''').strip())
